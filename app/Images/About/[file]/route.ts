...
const aliases: Record<string, string[]> = {
  classrooms: ["class"],
  "solar-fans": ["solar", "fan"],
  furniture: ["furniture"],
  washrooms: ["washroom", "wash"],
  "book-bank-library": ["book", "library"],
...
export async function GET(
  ...
  for (const root of roots) {
    try {
      ...
      const imageFiles = entries.filter((entry) => Boolean(contentTypes[extname(entry).toLowerCase()]));
